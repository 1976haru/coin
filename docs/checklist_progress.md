...
마지막 업데이트: 2026-05-18 (#20 Exchange Adapter Interface 확장 — 스펙 메서드 alias `fetch_price`/`get_balance` 추가, base `place_order` 가 LIVE 모드 + ENABLE_LIVE_TRADING=false 조합을 사전 거부(`live_not_wired` route), MockExchangeAdapter 가 client_order_id/idempotency_key 기반 idempotent 처리 + 잔고부족 REJECTED + LIMIT/MARKET 구분 + LIVE mode dict 거부 + audit secret sanitize. `tests/test_exchange_adapter.py` 가 21 → 53 케이스로 확장 — spec 메서드 / client_order_id / 잔고부족 / LIVE 거부 / audit secret 부재 / Upbit·OKX stub disabled / Strategy·Agent·Market 모듈 place_order 직접 호출 부재 / brokers 네트워크 import 부재. `docs/exchange_adapter_interface.md` 신규. backend 1687 passed / 1 skipped, frontend 변경 없음. 21번 이후 미수정.)
