...
마지막 업데이트: 2026-05-18 (#19 Trend/News/Theme Signals 확장 — DB-backed `ThemeSignal` 모델 + alembic `0005_theme_signals.py` 마이그레이션, `ThemeSignalCollector` + `ThemeProvider` Protocol + `MockThemeProvider`(외부 호출 0, 결정론 9개 fixture, trend/news/disclosure/theme/macro_fx 다중 source), `infer_risk_flags` (regulatory/exchange_risk/delisting/hype/macro_fx/high_news/review_required/context_only — 화이트리스트 강제 + BUY/SELL/ENTER/EXIT 절대 부재), `(source,provider,signal_id)`/`(source,provider,content_hash)` UNIQUE dedup, `used_for_order` + `direct_order_allowed` 영구 False, `ThemeFilter` + `ThemeContextBuilder` (read-only — candidate_filter_ok / candidate_filter_review_required 만 사용, action 컬럼 부재), REST `GET /api/theme-signals` + `POST /api/theme-signals/collect` (admin) + `GET /api/theme-signals/context` + `GET /api/theme-signals/sources` + `POST /api/theme-signals/filter`, ThemeInsightAgent.decide() 가 ctx["theme_context"] 첨부, `docs/theme_signals.md` 신규. backend 1655 passed / 1 skipped, frontend 변경 없음. 20번 이후 미수정.)
