...
마지막 업데이트: 2026-05-18 (#22 OKX Adapter 확장 — `app/brokers/okx_public.py` (transport-주입 OkxPublicClient: instruments/ticker/orderbook/candles/funding-rate, public path 화이트리스트), `app/brokers/okx_rate_limit.py` (`parse_okx_api_error`/`is_okx_rate_limit_error`/`should_throttle_okx`/`OkxRateLimitState` — 코드 50011 감지, sleep 주입), `app/brokers/okx_account.py` (OkxAccountClient — key+secret+passphrase 셋 모두 필요, transport 주입 필요, secret 미보관/미노출, `/api/v5/account/balance`/`positions` 만 화이트리스트), `app/brokers/okx_trade.py` (OkxTradeClient disabled stub + OkxPaperOrderClient 결정론적 spot/swap PAPER 엔진 — leverage/margin/reduce_only 입력은 받되 실제 미적용, LIVE mode 거부, client_order_id idempotent, 잔고 부족 REJECTED, audit secret sanitize). OkxAdapter 에 module-level `normalize_okx_inst_id`/`infer_okx_inst_type`/`to_internal_symbol` 헬퍼 + `public_client` 주입 옵션 추가. `tests/test_okx_adapter.py` 가 28 → 88 케이스로 확장 — instrument 정규화(spot/swap/option-rejection) / 50011 rate limit / public client transport injection / account gated / trade disabled / paper spot/swap MARKET-FILLED·LIMIT-ACCEPTED·잔고부족·LIVE 거부 / audit secret 부재 / Strategy·Agent 직접 호출 부재 / brokers 내 requests·httpx·jwt·hmac 부재 / 실제 trade endpoint URL literal 부재 / frontend OKX 키 부재. `docs/okx_adapter.md` 신규. backend 1793 passed / 1 skipped, frontend 변경 없음. 23번 이후 미수정.)
