...
마지막 업데이트: 2026-05-18 (#23 Binance Adapter (research/skeleton) — `app/brokers/binance_public.py` (transport-주입 BinancePublicClient: exchangeInfo/ticker/24hr/depth/klines/time, public path 화이트리스트 강제, native `BTCUSDT` 형식 강제 — slash/dash 거부), `app/brokers/binance_rate_limit.py` (`parse_binance_used_weight` — `X-MBX-USED-WEIGHT(-1M)` + order count 헤더 파싱 + sleep 주입), `app/brokers/binance_account.py` (BinanceAccountClient — 본 단계는 read-only skeleton 이라 credentials 무관 *모든* 메서드 disabled, secret 미보관/미노출, 출금 메서드 부재), `app/brokers/binance_trade.py` (BinanceTradeClient disabled stub — place/cancel/get/leverage/margin/futures/margin trading 7개 동작 모두 ExchangeAdapterDisabledError, DISABLED_REASON='binance_live_trading_disabled_until_regulatory_review', signing 코드 부재, 실제 trade endpoint URL literal 부재). BinanceAdapter 에 module-level `normalize_binance_symbol`/`to_internal_symbol`/`is_supported_binance_quote` 헬퍼 + `public_client` 주입 옵션 추가. `tests/test_binance_adapter.py` 가 27 → 84 케이스로 확장 — symbol native(BTC→BTCUSDT, BTC-USDT→BTCUSDT, futures/perp 거부) / used-weight 헤더 / public client transport / account 모두 disabled / trade 모두 disabled + regulatory reason / Strategy·Agent 직접 호출 부재 / brokers 내 requests·httpx·jwt·hmac·ccxt(binance_public)·python-binance/binance-connector 부재 / 실제 trade·account endpoint URL literal 부재 / `X-MBX-APIKEY` literal 부재 / frontend BINANCE 키 부재. `docs/binance_adapter.md` research doc 신규 (규제/지역 제한 명시 + 권장 host `data-api.binance.vision`). backend 1845 passed / 1 skipped, frontend 변경 없음. 24번 이후 미수정.)
