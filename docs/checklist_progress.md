...
마지막 업데이트: 2026-05-18 (#21 Upbit Adapter 확장 — `app/brokers/upbit_public.py` (transport-주입 UpbitPublicClient: markets/ticker/orderbook/candles-minutes/trades-ticks, public path 화이트리스트 강제), `app/brokers/upbit_rate_limit.py` (parse_remaining_req + should_throttle + RateLimitState — sleep 주입 가능), `app/brokers/upbit_account.py` (UpbitAccountClient — credentials/transport 양쪽 없으면 PermissionError, secret 은 보관/repr 노출 안 함, /v1/accounts 만 화이트리스트), `app/brokers/upbit_order.py` (UpbitOrderClient — 모든 동작 ExchangeAdapterDisabledError, JWT/HMAC signing 부재, credentials 인자도 폐기). UpbitAdapter 에 module-level `normalize_upbit_market`/`to_internal_symbol`/`is_krw_market` 헬퍼 + `public_client` 주입 옵션 추가 (legacy pyupbit 경로 보존). `tests/test_upbit_adapter.py` 가 30 → 73 케이스로 확장 — public client transport injection / Remaining-Req 파싱 / 안전 throttle / account gated / order disabled / KRW/USDT/BTC 마켓 정규화 / Strategy·Agent 직접 호출 부재 / brokers 내 requests·httpx·jwt·hmac import 부재 / 출금 메서드 부재 / 주문 endpoint URL 부재 / frontend 에 UPBIT 키 노출 부재. `docs/upbit_adapter.md` 신규. backend 1733 passed / 1 skipped, frontend 변경 없음. 22번 이후 미수정.)
