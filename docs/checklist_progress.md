...
마지막 업데이트: 2026-05-18 (#18 Exchange Notices 확장 — DB-backed `ExchangeNotice` 모델 + alembic `0004_exchange_notices.py` 마이그레이션, `NoticeCollector`/`NoticeSource` Protocol/`MockNoticeSource` + 8개 notice_type 분류기 (DEPOSIT_WITHDRAWAL_SUSPENSION/CAUTION/DELISTING/LISTING/MAINTENANCE/TRADING_SUSPENSION/POLICY/OTHER) + INFO/WARNING/HIGH/CRITICAL severity + (exchange,notice_id)/(exchange,content_hash) UNIQUE dedup, `NoticeContextBuilder` read-only Agent context + symbol risk flags + `direct_order_allowed=false` 영구, REST `POST /api/notices/collect` (admin) + `GET /api/notices/context` + `/api/notices/types` + 기존 `GET /api/notices` 응답 확장(exchange_notices/summary), ThemeInsightAgent.decide() 가 notice_context 첨부, `docs/exchange_notices.md` 신규. backend 1611 passed / 1 skipped, frontend 변경 없음. 19번 이후 미수정.)
