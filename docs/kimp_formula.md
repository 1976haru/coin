...
## 15. KimpStrategy / KimpAgent / KimpRiskGuards 와의 관계
...
- **#35 KimpRiskGuards** ([`docs/kimp_guards.md`](kimp_guards.md)) — `KimpResult` 의
  `fx_anomaly` / `risk_flags` 를 직접 입력으로 받는 8개 가드 (`check_fx_risk` 등).
  `evaluate_kimp_guards` 가 `KimpGuardDecision` 을 반환하며 ALLOW_CANDIDATE /
  REVIEW_REQUIRED / BLOCK_CANDIDATE 라벨로 KimpStrategy / KimpAgent 가 후보를
  필터링한다. `direct_order_allowed=False` 영구.
